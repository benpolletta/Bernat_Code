function summed_MI_figure_by_state_stats_by_subject_plot_by_channel_temp(significance, bands_plotted)
...
if ~isempty(find(bands_plotted == 5))
    c_order(bands_plotted == 5, :) = [0 1 0]; end
if ~isempty(find(bands_plotted == 6))
    c_order(bands_plotted == 6, :) = [1 0 0]; end

clear titles xlabels ylabels

All_BP_stats = nan(max(timestep_lengths), no_channels, no_bands, no_states, no_drugs, 5, no_norms, no_timesteps);
...
for n=1:no_norms
    ...
    for t = 1:no_timesteps
        ...
        for c=1:no_channels
            ...
            All_BP_stats(1:timestep_lengths(t), c, :, :, :, :, n, t) = BP_stats_new(1:timestep_lengths(t), :, :, :, :, :); % [1 4 5] are where the median, mean, and std are.
            ...
            for s = 1:no_stats
                ...
                for c = 1:no_channels
                    ...
                    for d = 2:no_drugs
                        ...
                        plot_stats = squeeze(All_BP_stats(1:timestep_lengths(t),...
                            c, bands_plotted, state, 1, :, n, t));
                        
                        if length(size(plot_stats)) == 2, plot_stats = reshape(plot_stats, [size(plot_stats, 1), 1, size(plot_stats, 2)]); end
                        
                        ax(c, d - 1) = subplot(no_channels, no_drugs - 1, (c - 1)*(no_drugs - 1) + d - 1);
                        
                        set(gca, 'NextPlot', 'add', 'ColorOrder', c_order) % , 'LineStyleOrder', {':', '-'})
                        
                        h = boundedline((1:timestep_lengths(t))', plot_stats(:, :, 1),...
                            prep_for_boundedline(-plot_stats(:, :, 2) + plot_stats(:, :, 1),...
                            plot_stats(:, :, 3) - plot_stats(:, :, 1)), 'cmap', c_order);
                    ...
                    for d = 2:no_drugs
                        ...
                        plot_test = double(All_BP_test(1:timestep_lengths(t), c, :, bands_plotted, d - 1, n, t, state)); %]; % drug_p_val_index(d) - 1)]; [nan(size(All_BP_test(:, :, bands_plotted(b), d - 1)))... % drug_p_val_index(d) - 1)))...
                        ...
                        if c == 1
                            ...
                            if d - 1 == 1
                                ...
                                legend(my_legend, 'Location', 'NorthEast', 'FontSize', 6)
                ...
                save_as_pdf(gcf,['ALL_summed_MI',norms{n},'_multichannel_', timesteps{t}, band_label, sig_label, '_', states{state}, '_stats_by_subject_plot_by_channel_boundedline']) %, 'orientation', 'portrait')
