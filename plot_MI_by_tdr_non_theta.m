function plot_MI_by_tdr_non_theta(drug, quantile_used, states)
...
for c = 1:length(criteria)
    ...
    theta_labels{c} = ['High ', criteria{c}];
    
end

for p = 1:length(pairs)
    
    theta_labels{length(criteria) + p} = ['High ', criteria{pairs(p, 1)}, ' & ', criteria{pairs(p, 2)}];
    
end

theta_labels{end + 1} = 'High \theta/\delta (peak) & \theta/\delta & delta & theta';
...
for s = 1:subj_num
    ...
    for d = 1:length(theta_labels)
        ...
        imagesc(phase_freqs, amp_freqs, reshape(median_subj_ntMI(:, d, s), no_afs, no_pfs))
    ...
    save_as_pdf(gcf, [subject, '_', drug, '_non_theta_MI_q', num2str(quantile_used), state_label])
...
for d = 1:no_thetas
    ...
    imagesc(phase_freqs, amp_freqs, reshape(median_ntMI(:, d), no_afs, no_pfs))
...
save_as_pdf(gcf, [drug, '_non_theta_MI_q', num2str(quantile_used), state_label])