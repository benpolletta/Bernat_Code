function NVP_theta_delta_timecourse_figure_by_state(state)

load('states')

state_index = find(strcmp(states, state));
...
for n=1:no_norms
    ...
    for c=1:no_channels
        ...
        load(['ALL_',channel_names{c},'/ALL_',channel_names{c},'_p0.99_IEzs_MI',...
            '/ALL_',channel_names{c},'_p0.99_IEzs_hrMI_hr_by_state_',drugs{3},'_cplot_data.mat'])
        
        pd_index = strcmp(cat2_labels, 'post1') | strcmp(cat2_labels, 'post2')...
            | strcmp(cat2_labels, 'post3') | strcmp(cat2_labels, 'post4');
        
        All_MI_cplot_data(:, :, :, c) = squeeze(MI_stats(:, :, state_index, pd_index, :, 1));
        
        %% Getting summed MI time series data.
        
        suffix = 'hrMI_6min_by_state';
        ...
        sMI_stats_new = permute(squeeze(BP_stats(:, state_index, :, :, :)), [2, 1, 3, 4]);
        ...
        suffix = ['_BP_', norms{n}, '6min_by_state'];
            
        load([ch_dir, '/', ch_dir, suffix, '.mat'])
        
        pd_index = (str2double(cat2_labels) >= min(short_6min_labels) & str2double(cat2_labels) <= max(short_6min_labels));
        
        BP_stats_new = permute(squeeze(BP_stats(:, state_index, :, :, :)), [2, 1, 3, 4]);
...
for n = 1 % 1:no_norms
    ...
    for d = 3 % 1:no_drugs
        ...
        set(h1, 'LineWidth', 2, 'Color', [1 0 0]), set(h2, 'LineWidth', 2, 'Color', [0 1 0])
        ...
        h = boundedline(short_BP_6min_labels'/60, plot_mean, prep_for_boundedline(plot_se), 'cmap', colormap);
...
save_as_pdf(gcf, sprintf('NVP_theta_delta_timecourse_%s', state))
...
for ch = 1:2
    ...
    for band = 1:2
        ...
        BP1 = All_BP_stats(hr1_index, channel_indices(ch), band, 2, d, n);
        
        BP2 = All_BP_stats(hr4_index, channel_indices(ch), band, 2, d, n);
...
save_as_pdf(gcf, sprintf('NVP_theta_delta_barplot_%s', state))
