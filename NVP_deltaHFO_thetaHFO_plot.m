function NVP_deltaHFO_thetaHFO_plot(drug, hour)

if nargin < 2, hour = []; end
if isempty(hour), hour = 4; end
hour = sprintf('post%d', hour);
...
load(['ALL_Occipital/ALL_Occipital_p0.99_IEzs_MI/',...
    'ALL_Occipital_p0.99_IEzs_hrMI_hr_', drug, '_cplot_data.mat'])

hour_indicator = strcmp(cat2_labels, hour);

subplot(2, 3, 1)

imagesc(phase_freqs, amp_freqs, MI_stats(:, :, :, hour_indicator, 1))

axis xy

colorbar

title({'All Occipital PAC'; ['Hr. ', hour(end), ' Post-Injection']; 'Median MI'}, 'FontSize', 16)
...
for lf = 1:2
    ...
    title({['Top Quartile for ', HFO_labels{lf}, ' PAC']; ['Hr. ', hour(end), ' Post-Injection']; 'Median MI'}, 'FontSize', 16)
...
load(['Occipital_', drug, '_thetaHFO_deltaHFO_multidrug_ds', num2str(ds_factor, '%.3g'), '.mat'])
...
save_as_pdf(gcf, [drug, '_dHFO_tHFO_figure'])
