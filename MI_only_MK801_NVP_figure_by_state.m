function MI_only_MK801_NVP_figure_by_state % (hi_hr, cplot_norm)
...
All_cplot_data = nan(no_afs, no_pfs, no_states, no_hr_periods, no_drugs, no_channels, no_norms);
...
if strcmp(hi_hr, 'independent')
    ...
    max_hr_indices = reshape(max_hr_indices, no_states, no_drugs, no_channels, no_norms);

elseif strcmp(hi_hr, 'drug')
   
    [~, max_hr_indices] = nanmax(nanmax(nanmax(nanmax(All_cplot_data(:, :, :, 5:end, :, :, :))), [], 5), [], 4);
    
    max_hr_indices = repmat(reshape(max_hr_indices, no_states, no_drugs, 1, no_norms), [1 1 no_channels 1]);
    
end

handle = nan(no_states, no_drugs, no_norms, no_stats);

All_cplot_for_plot = nan(no_afs, no_pfs, no_states, no_drugs + 1, no_stats, no_channels, no_norms);
...
elseif strcmp(cplot_norm, '_col')
    ...
    max_by_drug = reshape(nanmax(nanmax(nanmax(All_cplot_for_plot)), [], 5), no_states, no_drugs + 1, no_stats, no_norms);
    
    min_by_drug = reshape(nanmin(nanmin(nanmin(All_cplot_for_plot)), [], 5), no_states, no_drugs + 1, no_stats, no_norms);
